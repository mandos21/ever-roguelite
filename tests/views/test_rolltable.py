...
class RollTableControllerTestCase(ControllerTestBase):
    ...
    def setUp(self):
        ...
        self.item = Item(
            name="Sword of Testing", description="A powerful sword", weight=10
        ).save()
    ...
    def test_get_rolltables(self):
        ...
        response = self.client.get(
            "/rolltables/", headers={"Authorization": f"Bearer {self.token}"}
        )
    ...
    def test_delete_rolltable(self):
        ...
        self.assertEqual(
            response_data["message"], "RollTable deleted successfully!"
        )

    def test_add_item_to_rolltable(self):
        data = {"items": [str(self.item.id)]}
        response = self.client.post(
            f"/rolltables/{self.rolltable.id}/add",
            json=data,
            headers={"Authorization": f"Bearer {self.token}"},
        )
    ...
    def test_add_to_nonexistent_rolltable(self):
        data = {"items": [str(self.item.id)]}
        response = self.client.post(
            "/rolltables/507f1f77bcf86cd799439011/add",
            json=data,
            headers={"Authorization": f"Bearer {self.token}"},
        )
    ...
    def test_add_without_token(self):
        data = {"items": [str(self.item.id)]}
        response = self.client.post(
            f"/rolltables/{self.rolltable.id}/add", json=data
        )
